...
public class Hospital{
     ...
     * The hospital map.
     */
    private Network<Integer> hospitalMap;
    /**
    ...
    /**
     * The edges in the hospital map.
     */
    private final UnorderedListADT<Edge> edges;
    ...
    public Hospital() {
        ...
        this.edges = new UnorderedLinkedList<>();
    }

    /**
     * Gets all the edges in the hospital map.
     *
     * @return A list of all the edges in the hospital map.
     */
    public UnorderedListADT<Edge> getEdges() {
        return edges;
    }
    ...
    public void printVisualMap() {
        ...
        for (int i = 0; i < rooms.size(); i++) {
            for (int j = 0; j < rooms.size(); j++) {
                if (hospitalMap.edgeExists(i, j) && graph.getEdge(i + "_" + j) == null) {
                    double weight = hospitalMap.getWeight(i, j);
                    String edgeId = i + "_" + j;
    ...
    private boolean hasPermission(Person person, Room room) {
        ...
        if (room.getAccess().contains(function)) {
            return true;
        } else {
            System.out.println("\nAccess denied. You do not have permission to enter this room.");
            return false;
        }
    }
    ...
    public Event readEvent() throws IOException {
        ...
        System.out.println(getAccessibleRooms(from.getId()));
        ...
        if (getAccessibleRooms(from.getId()).contains(to) && hasPermission(person, to)) {
            return new Event(person, from, to, LocalDateTime.now());
        } else {
            while (!getAccessibleRooms(from.getId()).contains(to) || !hasPermission(person, to)) {
    ...
    public void findClosestExit(int startRoom) {
        ...
        while (exitIterator.hasNext()) {
            ...
            double weight = hospitalMap.shortestPathWeight(startRoom, exitRoom);

            Iterator<Integer> path = findShortestPath(startRoom, exitRoom);
            System.out.println("\n---------------------------------------------------------");
            System.out.println("\nPath from room " + startRoom + " to exit " + exitRoom);
            System.out.println("Weight: " + weight);
            System.out.println("\nMove from " + getRoomById(startRoom).getName());
            while (path.hasNext()) {
                Room room = getRoomById(path.next());
                System.out.println(" to " + room.getName());
            }
            System.out.println("\n----------------------------------------------------------");
        }
    }
    ...
    private UnorderedListADT<Room> getExits() {
        ...
        while (edgeIterator.hasNext()) {
            ...
            Room room1 = getRoomById(edge.getRoom1());
            Room room2 = getRoomById(edge.getRoom2());
     ...
     */
    private Iterator<Integer> findShortestPath(int startVertex, int endVertex) {
        return hospitalMap.findShortestPath(startVertex, endVertex, hospitalMap);
    }
     ...
     */
    public UnorderedListADT<Room> getAccessibleRooms(int currentRoomId) {

        UnorderedListADT<Room> accessibleRooms = new UnorderedLinkedList<>();
        Iterator<Edge> edgeIterator = edges.iterator();
        while (edgeIterator.hasNext()) {
            Edge edge = edgeIterator.next();
            if (edge.getRoom1() == currentRoomId) {
                Room room = getRoomById(edge.getRoom2());
                if (accessibleRooms.isEmpty() == true) {
                    accessibleRooms.addToRear(room);
                } else if (!(accessibleRooms.contains(room))) {
                    accessibleRooms.addToRear(room);
                }
            }
            if (edge.getRoom2() == currentRoomId) {
                if (accessibleRooms.isEmpty() == true) {
                    accessibleRooms.addToRear(getRoomById(edge.getRoom1()));
                } else if (!(accessibleRooms.contains(getRoomById(edge.getRoom1())))) {
                    accessibleRooms.addToRear(getRoomById(edge.getRoom1()));
                }
            }
        }
